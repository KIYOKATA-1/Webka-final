...
app.post('/regist', async (req, res) => {
    ...
    try {
        ...
        const mailOptions = {
            ...
            html: '<b>Congratulations! You have successfully registered.</b>' 
        };
...
app.post('/updateparfume/:id', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { name, genre, price } = req.body;
        const updatedParfume = await Parfume.findByIdAndUpdate(req.params.id, { name, genre, price }, { new: true });
        ...
        console.error(error);
        res.status(500).send('Error updating perfume');
    }
});
